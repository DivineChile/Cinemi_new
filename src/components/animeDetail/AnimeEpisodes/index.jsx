    ...
    <section className="bg-(--neutral-color) pb-20 text-white">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-4 mb-6">
          ...
          {/* Filters Control Interface row */}
          <div className="flex flex-wrap items-center gap-3 font-[Inter] text-[13px]">
            {/* Provider Switcher Dropdown Menu selection */}
            <div className="flex items-center w-full md:w-fit gap-1.5 bg-white/5 border border-white/10 rounded-lg px-3 py-3 md:py-1.5 cursor-pointer">
              ...
              >
                {Object.keys(episodeData.providers || {}).map((providerKey) => (
                  <option
                    key={providerKey}
                    value={providerKey}
            ...
            {/* Sub / Dub Action Toggle Switch Buttons */}
        ...
        {/* This row only appears on the screen if the anime exceeds 100 total episodes */}
        {episodeChunks.length > 1 && (
          <div className="flex flex-col gap-2.5 mb-6 font-[Inter]">
            <span className="text-[13px] text-[#a1a1a1] font-semibold flex items-center gap-1.5 uppercase tracking-wider">
              <SlidersHorizontal size={13} className="text-(--brand-color)" />{" "}
              Select Episode Range:
            </span>
            <div className="flex flex-wrap gap-2">
              {episodeChunks.map((chunk, index) => {
                const startEP = index * EPISODES_PER_PAGE + 1;
                // Make sure the last chunk range ends precisely at the true last episode index number
        ...
        {/* 3. DYNAMIC CARD RESPONSIVE CONTENT GRID TRACK */}
